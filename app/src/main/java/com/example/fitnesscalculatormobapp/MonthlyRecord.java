...
public class MonthlyRecord extends AppCompatActivity {
    ...
    private void showMenu(View v) {
        PopupMenu popupMenu = new PopupMenu(MonthlyRecord.this, v);
        popupMenu.getMenuInflater().inflate(R.menu.popup_menu, popupMenu.getMenu());
        popupMenu.setOnMenuItemClickListener(new PopupMenu.OnMenuItemClickListener() {
            @Override
            public boolean onMenuItemClick(MenuItem menuItem) {
                if (menuItem.getItemId() == R.id.measurements) {
                    Intent intent = new Intent(MonthlyRecord.this, MyMeasurements.class);
                    startActivity(intent);
                    finish();
                }
                if (menuItem.getItemId() == R.id.weekly) {
                    Intent intent = new Intent(MonthlyRecord.this, WeeklyRecord.class);
                    startActivity(intent);
                    finish();
                }
                if (menuItem.getItemId() == R.id.monthly) {
                    Intent intent = new Intent(MonthlyRecord.this, MonthlyRecord.class);
                    startActivity(intent);
                    finish();
                }
                return false;
